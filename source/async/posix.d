...
import async.events;
import async.internals.path;
import core.sys.posix.signal;
import async.posix2;
import async.internals.logging;
...
package:
		...
		auto waitForEvents(Duration timeout)
		{
			static if (EPOLL) {
				...
				if (timeout != -1.seconds) {
					...
					auto tspec = async.internals.kqueue.timespec(secs, ns);
	...
	long send(in fd_t fd, in ubyte[] data)
	{
		static if (LOG) try log("Send to FD: " ~ fd.to!string); catch {}
		m_status = StatusInfo.init;
		import async.internals.socket_compat : send;
		retry:
			auto ret = cast(long)send(fd, cast(const(void)*) data.ptr, data.length, 0);
	...
	{
		import async.internals.socket_compat : recvmsg, msghdr, iovec, sockaddr_storage;
	...
	size_t sendMsg(in fd_t fd, NetworkMessage* msg) {
		import async.internals.socket_compat : sendmsg;
	...
	long recvFrom(in fd_t fd, ubyte[] data, ref NetworkAddress addr)
	{
		...
		retry:
			...
			auto ret = cast(long)recvfrom(fd, cast(void*) data.ptr, data.length, 0, addr.sockAddr, &addrLen);
	...
	long sendTo(in fd_t fd, in ubyte[] data, in NetworkAddress addr)
	{
		...
		retry:
			auto ret = cast(long)sendto(fd, data.ptr, data.length, 0, addr.sockAddr, addr.sockAddrLen);
	...
	private bool closeRemoteSocket(fd_t fd, bool forced) {
		...
		import async.internals.socket_compat : shutdown, SHUT_WR, SHUT_RDWR, SHUT_RD;
		if (forced)
...
private:
	...
	auto attemptConnectionAcceptance(AsyncSocket socket)
	{
		...
		import async.internals.socket_compat : accept, accept4, sockaddr_storage, socklen_t;
	...
	{
		import async.internals.socket_compat : recv, MSG_PEEK;
	...
	{
		import async.internals.socket_compat : AF_INET, AF_INET6, socklen_t, accept4, accept;
	...
	{
		import core.sys.posix.fcntl : O_NONBLOCK;
		import async.internals.socket_compat : accept, accept4, sockaddr, socklen_t;
	...
	bool initUDPSocket(fd_t fd, AsyncUDPSocket ctxt, UDPHandler del)
	{
		...
		else /* if KQUEUE */
		{
			...
			nothrow void deregisterEvent() {
				...
				kevent(m_kqueuefd, &(_event[0]), 2, null, 0, cast(async.internals.kqueue.timespec*) null);
			}
